...
export const Laitela = {
  ...
  maxAllDMDimensions(maxTier) {
    ...
    // Check if there are any dimensions before trying to access them
    if (unlockedDimensions.length === 0) return;
    
    // Make sure we don't try to access dimensions that don't exist
    const actualMaxTier = Math.min(maxTier, unlockedDimensions.length);
    
    for (let i = 0; i < actualMaxTier; i++) {
      if (unlockedDimensions[i]) {
